...
/// 打开excel
pub fn open_workbook_auto_buff(buff: Vec<u8>, suffix: &str) -> Result<Sheets, Error> {
    match suffix {
        "xls" => Ok(Sheets::XlsBuff(crate::open_workbook_buff(buff)?)),
        "xlsx" => Ok(Sheets::XlsxBuff(crate::open_workbook_buff(buff)?)),
        _ => Err(Error::Msg("Cannot detect file format")),
    }
}

/// open_workbook_buff(vec![0,1,2].as_slice());
fn open_workbook_buff<R: Reader<RS = BufReader<Cursor<Vec<u8>>>>>(
    buff: Vec<u8>,
) -> Result<R, R::Error> {
    let buff = Cursor::new(buff);
    let buff = BufReader::new(buff);
    R::new(buff)
}

// #[test]
// fn test_open_workbook_buff(){
//     open_workbook_buff(vec![0,1,2].as_slice());
// }
