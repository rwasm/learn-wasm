...
#[wasm_bindgen]
pub fn chasmStart() -> Result<(), JsValue> {
	let window = web_sys::window().unwrap();
    let document = window.document().unwrap();
    let canvas = document.get_element_by_id("chasm").unwrap();
	let canvas: web_sys::HtmlCanvasElement = canvas.dyn_into::<web_sys::HtmlCanvasElement>()?;

	// get window width & height
	// https://github.com/rustwasm/wasm-bindgen/issues/2231
	let win_width = window.inner_width()?.as_f64().ok_or(JsValue::NULL)? as u32;
	let win_height = window.inner_height()?.as_f64().ok_or(JsValue::NULL)? as u32;
