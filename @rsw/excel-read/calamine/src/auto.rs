...
pub enum Sheets {
    ...
    /// Xls reader
    XlsBuff(Xls<BufReader<Cursor<Vec<u8>>>>),

    /// Xlsx reader
    XlsxBuff(Xlsx<BufReader<Cursor<Vec<u8>>>>),
...
impl Reader for Sheets {
    ...
    fn vba_project(&mut self) -> Option<Result<Cow<'_, VbaProject>, Self::Error>> {
        match *self {
            Sheets::XlsBuff(ref mut e) => e.vba_project().map(|vba| vba.map_err(Error::Xls)),
            Sheets::XlsxBuff(ref mut e) => e.vba_project().map(|vba| vba.map_err(Error::Xlsx)),
    ...
    fn metadata(&self) -> &Metadata {
        match *self {
            Sheets::XlsBuff(ref e) => e.metadata(),
            Sheets::XlsxBuff(ref e) => e.metadata(),
    ...
    fn worksheet_range(&mut self, name: &str) -> Option<Result<Range<DataType>, Self::Error>> {
        match *self {
            Sheets::XlsBuff(ref mut e) => e.worksheet_range(name).map(|r| r.map_err(Error::Xls)),
            Sheets::XlsxBuff(ref mut e) => e.worksheet_range(name).map(|r| r.map_err(Error::Xlsx)),
    ...
    fn worksheet_formula(&mut self, name: &str) -> Option<Result<Range<String>, Self::Error>> {
        match *self {
            Sheets::XlsBuff(ref mut e) => e.worksheet_formula(name).map(|r| r.map_err(Error::Xls)),
            Sheets::XlsxBuff(ref mut e) => {
                e.worksheet_formula(name).map(|r| r.map_err(Error::Xlsx))
            }
    ...
    fn worksheets(&mut self) -> Vec<(String, Range<DataType>)> {
        match *self {
            Sheets::XlsBuff(ref mut e) => e.worksheets(),
            Sheets::XlsxBuff(ref mut e) => e.worksheets(),
